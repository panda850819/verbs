...
Read the `skills/meta/writing-great-skills/SKILL.md` scorecard axes. Resolve the target: `skills/<bucket>/<name>/SKILL.md` (bucket = engineering|productivity|writing|meta). Read it whole, plus any sibling reference files it points at. `--eval all` fans out one sub-agent per skill (hot/cold rule — never score the whole corpus in one hot context).
