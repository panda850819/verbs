...
| Signal | Invoke |
|---|---|
| The message itself names a map — start or resume a decision map / 建立 map / 繼續 map / names an existing map file | `wayfinder` (interviews and charts the map itself, then works one frontier entry per session) |
| Fuzzy idea / scope not locked / 想討論 / a large effort with no map yet | `grill` (drills, then routes large foggy work to `wayfinder`, spec-sized to `to-spec`, smaller to a local close; say "quick" for a chat-only log) |
